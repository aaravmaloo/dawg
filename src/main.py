        ...
        if total_dogs > 0 or total_value > 0:
            ...
            if member:
                ...
                try:
                    ...
                    user_display = user_obj.name if user_obj else f"User ID: {user_id_str}"
                except Exception:
                    user_display = f"User ID: {user_id_str}"
            leaderboard_entries.append({"name_mention": user_display, "total_value": total_value, "total_dogs": total_dogs})
...
@bot.tree.command(name="changespawntimings", description="Admin: Change the min/max dog spawn interval (in seconds) for this server.")
@app_commands.describe(min_seconds="Minimum seconds between spawns (e.g. 60)", max_seconds="Maximum seconds between spawns (e.g. 300)")
@app_commands.checks.has_permissions(administrator=True)
async def changespawntimings_slash(interaction: discord.Interaction, min_seconds: app_commands.Range[int, 10, 86400], max_seconds: app_commands.Range[int, 10, 86400]):
    if not interaction.guild:
        await interaction.response.send_message("Server only.", ephemeral=True)
        return
    ...
    save_guild_data(gid, guild_data, interaction.guild.name)
    await interaction.response.send_message(f"✅ Dog spawn timings updated: min {min_seconds}s, max {max_seconds}s.")
